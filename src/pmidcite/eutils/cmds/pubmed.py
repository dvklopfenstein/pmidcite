...
class PubMed(EntrezUtilities):
    ...
    #### MOVED TO: pmidcite/eutils/cmds/esearch
    #### def dnld_query_pmids(self, query, num_ids_p_epost=10):
    ####     """Searches an PubMed for a user query, writes resulting entries into one file."""
    ...
    ####     if rsp_dct is None:
    ####         if self.log:
    ####             self.log.write('No PubMed entries found: {Q}\n'.format(Q=query))
    ####             self.log.flush()
    ####         return []
    ...
    def dnld_wr1_per_pmid(self, pmid_nt_list, num_ids_p_epost=10):
        ...
        self.esearch.dnld_wr1_per_pmid(pmid_nt_list, 'pubmed', num_ids_p_epost)
    ...
    def get_pmid_nt_list(pmids, force_download, dir_pubmed):
        ...
        self.get_pmid_nt_list(pmids, 'pubmed', force_download, dir_pubmed)
    #### @staticmethod
    #### def get_pmid_nt_list(pmids, force_download, dir_pubmed):
    ####     """Get list of PubMed entries: Title, abstract, authors, journal, MeSH"""
    ####     nts = []
